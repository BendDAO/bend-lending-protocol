...
export const setAggregatorsInReserveOracle = async (
  ...
  for (const [assetSymbol, assetAddress] of Object.entries(allAssetsAddresses) as [
    string,
    tEthereumAddress
  ][]) {
    ...
    const [, aggAddress] = (Object.entries(aggregatorsAddresses) as [string, tEthereumAddress][])[
      aggAddressIndex
    ];
...
export const setPricesInNFTOracle = async (
  ...
  for (const [assetSymbol, price] of Object.entries(prices) as [string, string][]) {
    ...
    await waitForTx(
      await nftOracleInstance.setAssetData(assetAddress, price, '1444004400', '10001')
    );
  }
};
