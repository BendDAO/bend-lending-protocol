...
makeSuite("LendPoolLoan: Modifiers", (testEnv: TestEnv) => {
  ...
  it("Tries to invoke initNft not being the Pool", async () => {
    ...
    await expect(loan.initNft(bayc.address, bBAYC.address)).to.be.revertedWith(CT_CALLER_MUST_BE_LENDING_POOL);
  });
  ...
  it("Tries to invoke repayLoan not being the Pool", async () => {
    const { deployer, bayc, bBAYC, bWETH, loan } = testEnv;
    await expect(loan.repayLoan(deployer.address, "1", bBAYC.address)).to.be.revertedWith(
      CT_CALLER_MUST_BE_LENDING_POOL
    );
  });
  ...
  it("Tries to invoke liquidateLoan not being the Pool", async () => {
    const { deployer, bayc, bBAYC, bWETH, loan } = testEnv;
    await expect(loan.liquidateLoan(deployer.address, "1", bBAYC.address)).to.be.revertedWith(
      CT_CALLER_MUST_BE_LENDING_POOL
    );
  });
});
