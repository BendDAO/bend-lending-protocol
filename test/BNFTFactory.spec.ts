...
makeSuite("BNFTRegistry", (testEnv: TestEnv) => {
  ...
  it("Creates the BNFT for duplicate NFT asset", async () => {
    ...
    await expect(
      bnftRegistry.createBNFT(bayc.address, extraParams)
  ...
  it("Creates the BNFT for nonexistent NFT asset", async () => {
    ...
    await waitForTx(
      await bnftRegistry.createBNFT(testNftAsset.address, extraParams)
    );
    ...
    const { bNftProxy, bNftImpl } = await bnftRegistry.getBNFT(
  ...
  it("Creates the BNFT with implement for duplicate NFT asset", async () => {
    ...
    await expect(
      bnftRegistry.createBNFTWithImpl(
        bayc.address,
        testBNFTImpl.address,
        extraParams
      )
