...
makeSuite('BNFTRegistry', (testEnv: TestEnv) => {
  ...
  it('Creates the BNFT for duplicate NFT asset', async () => {
    ...
    await expect(bnftRegistry.createBNFT(bayc.address, extraParams)).to.be.revertedWith(
  ...
  it('Creates the BNFT with implement for nonexistent NFT asset', async () => {
    ...
    await waitForTx(
      await bnftRegistry.createBNFTWithImpl(testNftAsset.address, testBNFTImpl.address, extraParams)
    );
  ...
  it('Creates the BNFT with implement for duplicate NFT asset', async () => {
    ...
    await expect(
      bnftRegistry.createBNFTWithImpl(bayc.address, testBNFTImpl.address, extraParams)
    ).to.be.revertedWith('BNFTRegistry: asset exist');
  });
  ...
  it('Upgrades the BNFT with implement for nonexistent NFT asset', async () => {
    ...
    await expect(
      bnftRegistry.upgradeBNFTWithImpl(testNftAsset.address, testBNFTImpl.address, [])
