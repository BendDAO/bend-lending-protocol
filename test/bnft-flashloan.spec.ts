...
makeSuite("BNFT: FlashLoan function", (testEnv: TestEnv) => {
  ...
  it("Mints BAYC into the BNFT", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;
  ...
  it("Takes BAYC flashloan using one token, returns the tokens correctly", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;

    const ownerBefore = await bayc.ownerOf(user0TokenId1);
    expect(ownerBefore).to.be.equal(bBYAC.address);
    const ownerBeforeB = await bBYAC.ownerOf(user0TokenId1);
    expect(ownerBeforeB).to.be.equal(users[0].address);

    await bBYAC.connect(users[0].signer).flashLoan(_mockFlashLoanReceiver.address, [user0TokenId1], []);

    const ownerAfter = await bayc.ownerOf(user0TokenId1);
    expect(ownerAfter).to.be.equal(bBYAC.address);
    const ownerAfterB = await bBYAC.ownerOf(user0TokenId1);
    expect(ownerAfterB).to.be.equal(users[0].address);
  });

  it("Takes BYAC flashloan using many tokens, returns the tokens correctly", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;

    const ownerBefore1 = await bayc.ownerOf(user0TokenId1);
    expect(ownerBefore1).to.be.equal(bBYAC.address);
    const ownerBeforeB1 = await bBYAC.ownerOf(user0TokenId1);
    expect(ownerBeforeB1).to.be.equal(users[0].address);

    const ownerBefore2 = await bayc.ownerOf(user0TokenId2);
    expect(ownerBefore2).to.be.equal(bBYAC.address);
    const ownerBeforeB2 = await bBYAC.ownerOf(user0TokenId2);
    expect(ownerBeforeB2).to.be.equal(users[0].address);

    await bBYAC.connect(users[0].signer).flashLoan(_mockFlashLoanReceiver.address, [user0TokenId1, user0TokenId2], []);

    const ownerAfter1 = await bayc.ownerOf(user0TokenId1);
    expect(ownerAfter1).to.be.equal(bBYAC.address);
    const ownerAfterB1 = await bBYAC.ownerOf(user0TokenId1);
    expect(ownerAfterB1).to.be.equal(users[0].address);

    const ownerAfter2 = await bayc.ownerOf(user0TokenId2);
    expect(ownerAfter2).to.be.equal(bBYAC.address);
    const ownerAfterB2 = await bBYAC.ownerOf(user0TokenId2);
    expect(ownerAfterB2).to.be.equal(users[0].address);
  });

  it("Takes BAYC flashloan, does not return all the tokens. (revert expected)", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;

    await _mockFlashLoanReceiver.setTokenIdNotToApprove(user0TokenId1);

    await expect(
      bBYAC.connect(users[0].signer).flashLoan(_mockFlashLoanReceiver.address, [user0TokenId1], [])
    ).to.be.revertedWith("ERC721: transfer caller is not owner nor approved");
  });

  it("Takes BAYC flashloan, does not return partly the tokens. (revert expected)", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;

    await _mockFlashLoanReceiver.setTokenIdNotToApprove(user0TokenId2);

    await expect(
      bBYAC.connect(users[0].signer).flashLoan(_mockFlashLoanReceiver.address, [user0TokenId1, user0TokenId2], [])
    ).to.be.revertedWith("ERC721: transfer caller is not owner nor approved");
  });
  ...
  it("Tries to take a flashloan using not owned tokens (revert expected)", async () => {
    ...
    await expect(
      bBYAC.connect(users[0].signer).flashLoan(_mockFlashLoanReceiver.address, [user1TokenId1], [])
    ).to.be.revertedWith("BNFT: caller is not owner");
  });

  it("Takes BAYC flashloan, simulating receiver execution failed (revert expected)", async () => {
    const { users, pool, bayc, bBYAC } = testEnv;
  ...
  it("tries to take a flashloan using non contract address as receiver (revert expected)", async () => {
    ...
    await expect(bBYAC.connect(users[0].signer).flashLoan(users[1].address, [user0TokenId1], [])).to.be.revertedWith(
      ""
    );
  });
