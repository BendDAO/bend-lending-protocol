...
makeSuite("opensea downpayment adapter tests", (testEnv: TestEnv) => {
  ...
  makeSuite("aave flash loan", (testEnv: TestEnv) => {
    ...
    before(async () => {
      ...
      await testEnv.nftOracle.setAssetData(nftToken.address, parseEther("100"));
    ...
    it("downpayment buy", async () => {
      const nftCollateralData = await testEnv.pool.getNftCollateralData(nftToken.address, testEnv.weth.address);
