...
makeSuite("BToken: Modifiers", (testEnv: TestEnv) => {
  ...
  it("Tries to invoke mint not being the Pool", async () => {
    ...
    await expect(bDai.mint(deployer.address, "1", "1")).to.be.revertedWith(CT_CALLER_MUST_BE_LENDING_POOL);
  });
  ...
  it("Tries to invoke mintToTreasury not being the Pool", async () => {
    ...
    await expect(bDai.mintToTreasury("1", "1")).to.be.revertedWith(CT_CALLER_MUST_BE_LENDING_POOL);
  });

  it("Tries to invoke transferUnderlyingTo not being the Pool", async () => {
    const { deployer, bDai } = testEnv;
    await expect(bDai.transferUnderlyingTo(deployer.address, "1")).to.be.revertedWith(CT_CALLER_MUST_BE_LENDING_POOL);
  });
});
