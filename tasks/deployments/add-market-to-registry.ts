...
task("add-market-to-registry", "Adds address provider to registry")
  ...
  .setAction(async ({ verify, addressesProvider, pool, deployRegistry }, DRE) => {
    ...
    const addressesProviderRegistry = (await getLendPoolAddressesProviderRegistry(providerRegistryAddress)).connect(
      signer
    );

    const addressesProviderInstance = await getLendPoolAddressesProvider(addressesProvider);
