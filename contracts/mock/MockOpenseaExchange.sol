...
contract ExchangeCore is ReentrancyGuarded, Ownable {
  ...
  uint256 private constant _CHAIN_ID = 31337; // just for mock
  ...
  function transferTokens(
    ...
    if (amount > 0) {
      require(ERC20(token).transferFrom(from, to, amount), "transfer token failed");
    }
  }
   ...
   */
  function hashOrder(Order memory order, uint256 nonce) internal pure returns (bytes32 hash) {
  ...
  function requireValidOrder(
    ...
    require(validateOrder(hash, order, sig), "sig error");
    return hash;
  }
  ...
  function calculateMatchPrice(Order memory buy, Order memory sell) internal view returns (uint256) {
    ...
    /* Require price cross. */
    require(buyPrice >= sellPrice, "buy price should bigger then sell price");
  ...
  function executeFundsTransfer(Order memory buy, Order memory sell) internal returns (uint256) {
    ...
    if (sell.paymentToken == address(0)) {
      ...
      require(msg.value >= requiredAmount, "msg value insufficent");
  ...
  function atomicMatch(
    ...
    require(ArrayUtils.arrayEq(buy.calldata, sell.calldata), "calldata not matched");
    ...
    require(proxy.proxy(sell.target, sell.howToCall, sell.calldata), "proxy call error");

    /* Static calls are intentionally done after the effectful call so they can check resulting state. */

    /* Handle buy-side static call if specified. */
    if (buy.staticTarget != address(0)) {
      require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata), "static call error");
    }

    /* Handle sell-side static call if specified. */
    if (sell.staticTarget != address(0)) {
      require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata), "static call error");
    }
...
contract Exchange is ExchangeCore {
  ...
  function validateOrder_(
    ...
    return validateOrder(hashToSign(order, nonces[order.maker]), order, Sig(v, r, s));
  }
...
contract MockOpenseaExchange is Exchange {
...
contract ProxyRegistry is Ownable {
  ...
  function mock(address exchage, address authImpl) external {
    contracts[exchage] = true;
    delegateProxyImplementation = authImpl;
  }
