...
contract BendProtocolDataProvider {
  ...
  {
    DataTypes.ReserveData memory reserve = ILendPool(ADDRESSES_PROVIDER.getLendPool())
    ...
    )
  {
    ...
    currentVariableDebt = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan())
  ...
  function getNftTokensAddresses(address asset) external view returns (address bNftAddress) {
    DataTypes.NftData memory reserve = ILendPool(ADDRESSES_PROVIDER.getLendPool()).getNftData(
      asset
    );
  ...
  {
    loanData.loanId = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan()).getCollateralLoanId(
      nftAsset,
      nftTokenId
    );
    if (loanData.loanId != 0) {
      DataTypes.LoanData memory data = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan()).getLoan(
        loanData.loanId
      );
      ...
      loanData.currentAmount = ILendPoolLoan(ADDRESSES_PROVIDER.getLendPoolLoan())
